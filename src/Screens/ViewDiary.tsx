...
import { addDoc, collection, updateDoc ,doc, setDoc, deleteDoc} from "firebase/firestore";
import { FIREBASE_AUTH, FIRESTORE_DB } from "../../firebaseConfig";
import { useNavigation } from "@react-navigation/native";



export default function ViewDiary({ route }: any) {
    ...
    const pickImage = async () => {
        ...
        if (!result.canceled) {
            ...
            richText.current.disabled = false;
        }
    }
    ...
    const onChange = ({ type }, selectedDate) => {
        if (type == "set") {
            ...
            if (Platform.OS === "android") {
                toggleDatePicker();
                setPageDate((date.getMonth()+1)+"/"+date.getDate()+"/"+ date.getFullYear());
            }
        }
        else {
            toggleDatePicker();
    ...
    const updatePage = async () => { 
        const doc1 = await updateDoc(doc(FIRESTORE_DB, 'Diary',item.keys), {
    ...
    useEffect(() => {
        const unsubscribe = navigation.addListener('focus', () => {
            ...
            console.log(item)
            if (item.emoji == "Happy") {
                setHappyFocused(true);
            } else if (item.emoji == "Normal") {
                setNormalFocused(true);
            } else if (item.emoji == "Tired") {
                setCryFocused(true);
            } else if (item.emoji == "Sad") {
                setSadFocused(true);
            } else {
                setLovelyFocused(true);
            }

            setPageDate(item.date)
            setEmoji(item.emoji);
            richText.current?.setContentHTML(item.data);
        });
        return unsubscribe;
            ...
            <View style={styles.Container}>
                <View style={styles.section2}>
                    <View style={{
                        ...
                        <TouchableOpacity style={{ alignSelf: "flex-end", marginLeft: "12%",marginBottom:10 }}
                            onPress={() => editbleSet()}>
                            
                                            {editable ? (
                                                <TouchableOpacity  onPress={() => updatePage()}>
                                        <Ionicons
                                        name="checkmark" size={45}
                                        color={Colors.main} 
                        ...
                        <TouchableOpacity style={{ alignSelf: "flex-end", marginLeft:20,marginBottom:10 }}
                            onPress={() => editbleSet()}>

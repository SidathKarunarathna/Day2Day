...
const AddIncomeCategoryScreen = ({ navigation: nav }: any) => {
    ...
    const addIncomeCategory = async () => {
        try {
            const doc = await addDoc(collection(FIRESTORE_DB, 'Income Categories'),{
            ...
            alert('Failed to add Income Category. Please try again.');
        }
    };
    ...
    const deleteCategory= async (item:any) => { 
        ...
        const doc1 = await deleteDoc(doc(FIRESTORE_DB, 'Income Categories',item.id));
        fetchCategories();
    };
    return (
        <View style={styles.Container}>
            ...
            <View style={{
                        ...
                        <Text style={[styles.Header,{width:"80%",marginTop:40}]}>Income Categories</Text>
                    </View>

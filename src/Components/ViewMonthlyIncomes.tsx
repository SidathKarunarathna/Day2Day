...
export default function ViewMonthlyIncomes({ navigation: nav }: any) {
  const months = [
  ...
  useEffect(() => {
    ...
    if (incomes.length > 0 && expenses.length > 0) {
        ...
        for (let i = 0; i < expenses.length; i++) {
            const item1 = expenses[i];
            const expenseDate = item1.date ? item1.date.toDate() : null;
            if (
                expenseDate &&
        ...
        for (let i = 0; i < incomes.length; i++) {
            ...
            const incomeDate = item.date ? item.date.toDate() : null;
            if (
                incomeDate &&
        ...
        incomesTotal += parseFloat(item.amount); 
            }
        }
  ...
  return (
    <View style={styles.Container}>
      <View style={styles.section}>
        <Text style={styles.Header}>{months[date.getMonth()]} Incomes</Text>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View>
              {
                item.date.toDate().getMonth() + 1 === date.getMonth() + 1 &&
                  item.date.toDate().getFullYear() === date.getFullYear() ? (
                  <View style={styles.iconsSection}>
        ...
        />
        <View style={styles.Container}>
          <Text style={styles.SubTopic2}>{`Total Incomes: Rs.${totalIncomes}.00`}</Text>
        </View>
